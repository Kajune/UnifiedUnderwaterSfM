...
struct camera {
	...
	double focus;
	Eigen::Vector3d tvec = Eigen::Vector3d(0, 0, 0);
	Eigen::Quaterniond rquat = Eigen::Quaterniond::Identity();
	...
	ray<T> castRay(const Eigen::Vector2t<T>& pos2D) const {
		...
		return ray<T>{ Eigen::Vector3t<T>(tvec), (rquat * pos3d).normalized(), pos2D };
	}

	template <typename T>
	Eigen::Vector2t<T> project(const Eigen::Vector3t<T>& pos3D) const {
		Eigen::Vector3t<T> proj = getCameraMatrix() * (rquat.conjugate() * (pos3D - Eigen::Vector3t<T>(tvec)));
		proj /= proj.z();
		return Eigen::Vector2t<T>{ proj.x(), proj.y() };
	}
	
	template <typename T>
	bool isInImagePlane(const Eigen::Vector2t<T>& pos2D) const {
		return 0 <= pos2D.x() && pos2D.x() < resolution.x() && 0 <= pos2D.y() && pos2D.y() < resolution.y();
	}

	template <typename T>
	bool isInImagePlane(const Eigen::Vector3t<T>& pos3D) const {
		return isInImagePlane(project(pos3D));
	}
};
